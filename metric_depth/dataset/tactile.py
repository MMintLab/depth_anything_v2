...
class BUBBLES_CROPPED(Dataset):
    ...
    def __getitem__(self, item):
        ...
        x_min, y_min, x_max, y_max = 55, 45, 105, 90
        bubble_image_cropped = bubble_image[:, y_min:y_max, x_min:x_max]
        image = (bubble_image_cropped - np.min(bubble_image_cropped)) / (np.max(bubble_image_cropped) - np.min(bubble_image_cropped))
...
class DIGITS(Dataset):
    ...
    def __getitem__(self, item):
        img_path = self.filelist[item]
        depth_path = img_path.replace('digits', 'digits_depth')
        
        digits_image = torch.load(img_path)['digit'].permute(0, 2, 3, 1).numpy()
        digits_image_ref = torch.load(img_path)['digit_ref'].permute(0, 2, 3, 1).numpy()
        image  = digits_image # - gelslim_image_ref
        image = (image - np.min(image)) / (np.max(image) - np.min(image))
